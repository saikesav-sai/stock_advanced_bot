    ...
    def load_data(self, symbol: str, start_date: str, end_date: str, instrument_key: str = None) -> pd.DataFrame:
        ...
        if len(df) == 0:
            ...
            if self.auto_fetch:
                ...
                # Add buffer days (account for weekends and holidays)
                days_to_fetch = max(days_diff + 10, 30)  # Minimum 30 days

                try:
                    # Fetch historical data
                    fetcher = UpstoxHistoricalFetcher()
                    fetcher.fetch_and_store_candles(instrument_key, days=days_to_fetch)
                    fetcher.close()
